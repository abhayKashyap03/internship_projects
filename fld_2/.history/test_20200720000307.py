...
def vid(video, out) :
    ...
    threshold = 0.7
    sum = 0
    video = cv2.VideoWriter(os.path.join(out, "result.mp4"), cv2.VideoWriter_fourcc(*'mp4v'), 15, (width, height))
    while True:
        ...
        sum += boxes.shape[0]
        cv2.imwrite(os.path.join(out, "img"+str(sum)+".jpg"), orig_image)
...
if args.type == 'img' :
    img(args.path, args.out)
elif args.type == 'vid' :
    vid(args.path, args.out)
elif args.type == 'folder' :
    folder(args.path, args.out)      
