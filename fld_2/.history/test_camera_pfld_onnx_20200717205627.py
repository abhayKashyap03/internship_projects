...
parser.add_argument('--type', choices=['img', 'vid'], required=True)
...
def folder(folder) :
    ...
    if os.path.splitext(files[0]) in image :
        for file in files :
            img(file)
    elif os.path.splitext(files[0]) in video :
        for file in files :
            vid(file)          
...
'''def img(image) :
    ...
    for i in range(boxes.shape[0]):
        ...
        orig_image = drawLandmark_multiple(image, new_bbox, landmark)
        cv2.imwrite("results/"+os.path.basename(image), orig_image)
'''
...
def img(image_path) :
    ...
    cv2.imwrite("results/"+os.path.basename(image_path), orig_image)



def vid(video) :
    if not os.path.exists("results/"+os.path.splitext(os.path.basename(video))[0]) :
        os.mkdir("results/"+os.path.splitext(os.path.basename(video))[0])
    # perform face detection and alignment from camera
    cap = cv2.VideoCapture(video)  # capture from camera
    threshold = 0.7
    sum = 0
    while True:
        ...
        for i in range(boxes.shape[0]):
            ...
            orig_image = drawLandmark_multiple(orig_image, new_bbox, landmark)
        sum += boxes.shape[0]
        cv2.imwrite("results/"+os.path.splitext(os.path.basename(video))[0]+"/img"+str(sum)+".jpg", orig_image)
