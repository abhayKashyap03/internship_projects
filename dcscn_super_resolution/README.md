...
python sr.py --file=/path/to/your/image

# apply super resolution with small model
python sr.py --file=/path/to/your/image --layers=8 --filters=96
```

## Apply to your own video

The program extracts creates frames from the video and applies the super resolution program on each frame, then compiles all the frames into a video.
'--video_path' : Location of input video
'--frame_out_dir' : Location where the frames are to be stored
'--video_out_dir' : Location where the final compiled video has to be stored

```
python test.py --video_path /path/to/your/video --frame_out_dir /path/to/output/frames --video_out_dir /path/to/output/video
```
